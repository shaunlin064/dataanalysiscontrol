    ...
    class ReviewController extends BaseController {
        ...
        public function view ()
        {
            ...
            $bonusColumns = [
                ...
                [ 'data' => 'campaign_name', 'render' => sprintf('<a href="http://%s/jsadwaysN2/campaign_view.php?id=${row.campaign_id}" target="_blank">${row.campaign_name}</a>',
                                                                 env('ERP_URL')) ],
...
//                dd($return);
        ...
        public function getAjaxData ( Request $request, $outType = 'echo' )
        {
            ...
            $hash = $this->gethash($dateStart, $dateEnd, $saleGroupIds, $userIds, $agencyIdArrays, $clientIdArrays,
            ...
            $cacheDataLastRecord->each(function ( $v ) use ( &$last_record_bonus_list ) {
            ...
            if ( !Cache::store('file')->has($cahceKey . 'filterData' . $hash) )
            {
                ...
                foreach ( $dateRange as $dateItem )
                {
                    if ( $saleGroupIds )
                    {
                        ...
                        {
                            /*抓出該月份該團隊userid*/
                            $tmpUserIds = $userIds->where('sale_groups_id', $saleGroupId)
                                ->where('set_date', $dateItem)
                                ->pluck('erp_user_id');
                            /*如果該月抓不到責任額 則抓上月資料*/
                            if ( $tmpUserIds->count() == 0 )
                            {
                                ...
                                $tmpUserIds = $userIds->where('sale_groups_id', $saleGroupId)
                                    ->where('set_date', $tmpdate->format('Y-m-01'))
                                    ->pluck('erp_user_id');
                            }

                            $tmpBonus = $tmpBonus->concat($bonus_list->where('set_date', substr($dateItem, 0, 7))
                                                              ->whereIn('erp_user_id', $tmpUserIds));
                            $tmpProgressList = $tmpProgressList->concat($progress_list->where('set_date',
                                                                                              substr($dateItem, 0, 7))
                                                                            ->whereIn('erp_user_id', $tmpUserIds));
                            ...
                            /*抓出該月份該團隊userid*/
                            $tmpUserIdslast = $userIds->where('sale_groups_id', $saleGroupId)
                                ->where('set_date', $tmpdatelast->format('Y-m-01'))
                                ->pluck('erp_user_id');
                            /*如果該月抓不到責任額 則抓上月資料*/
                            if ( $tmpUserIdslast->count() == 0 )
                            {
                                ...
                                $tmpUserIdslast = $userIds->where('sale_groups_id', $saleGroupId)
                                    ->where('set_date', $tmpdate2->format('Y-m-01'))
                                    ->pluck('erp_user_id');
                            }

                            $tmpBonusLastRecord = $tmpBonusLastRecord->concat($last_record_bonus_list->where('set_date',
                                                                                                             $tmpdatelast->format('Y-m'))
                                                                                  ->whereIn('erp_user_id',
                                                                                            $tmpUserIdslast));
                ...
                $last_record_bonus_list = $this->getFilterData($tmpBonusLastRecord->toArray(), $agencyIdArrays,
                                                               $clientIdArrays, $mediaCompaniesIdArrays,
                                                               $mediasNameArrays);
                ...
                $last_record_bonus_list->groupBy('set_date')
                    ->map(function ( $v, $k ) use ( &$chartFinancialBarLastRecord ) {
                        $tmpDate = new DateTime($k);
                ...
                [ $customerPrecentageProfit, $customerProfitData, $customerGroupsProfitData ] = $this->getCustomerAllAnalysis($bonus_list,
                                                                                                                              $dateRange);
                [ $mediaCompaniesProfitData, $mediasProfitData, $saleChannelProfitData ] = $this->getMediaAllAnalysis($bonus_list);
                $chart_bar_max_y = collect(array_merge($chartFinancialBar['totalIncome'],
                                                       $chartFinancialBarLastRecord['totalIncome']))->max();
                Cache::store('file')
                    ->put($cahceKey . 'filterData' . $hash,
                          [ $customerPrecentageProfit, $customerProfitData, $customerGroupsProfitData, $mediaCompaniesProfitData, $mediasProfitData, $saleChannelProfitData, $chartMoneyStatus, $chartFinancialBar, $bonus_list, $progress_list, $chartFinancialBarLastRecord, $chart_bar_max_y ],
                        ( $cacheTime * 3600 ));

            } else
            {

                [ $customerPrecentageProfit, $customerProfitData, $customerGroupsProfitData, $mediaCompaniesProfitData, $mediasProfitData, $saleChannelProfitData, $chartMoneyStatus, $chartFinancialBar, $bonus_list, $progress_list, $chartFinancialBarLastRecord, $chart_bar_max_y ] = Cache::store('file')
                    ->get($cahceKey . 'filterData' . $hash);
            ...
            /*個月stack*/
//            $bonusCharBarStack = $bonus_list->groupBy('set_date')->map(function ( $v, $k ) {
...
//            })->toArray();
        ...
        private function getCustomerGroupsProfit ( Collection $bonus_list, $dateRange )
        {
            ...
            $customerGroupsProfit = $customerGroup->map(function ( $v, $k ) use ( $bonus_list, $receiptTimesData ) {
                ...
                $receiptTimes = $receiptTimesData->whereIn('agency_id', $v['customer']['agency'])
                    ->concat($receiptTimesData->whereIn('client_id', $v['customer']['client']))
                    ->sum('receipt_count_times');
        ...
        {
            [ $mediaCompaniesProfitData, $mediasProfitData ] = [ [], [] ];
            /*部份資料前置時有做 number_format 這邊做還原*/
            $bonus_list = $bonus_list->map(function ( $v, $k ) {
                $v['profit'] = str_replace(',', '', $v['profit']);
                return $v;
         ...
         * @return string
         */
        private function gethash ( $dateStart, $dateEnd, $saleGroupIds, $userIds, array $agencyIdArrays, array $clientIdArrays, array $mediaCompaniesIdArrays, array $mediasNameArrays ): string
        {
            $hash = '';

            foreach ( [ $dateStart, $dateEnd, $saleGroupIds, $userIds, $agencyIdArrays, $clientIdArrays, $mediaCompaniesIdArrays, $mediasNameArrays ] as $key => $item )
            {
                if ( $key <= 1 )
                {
                    $hash .= $item;
